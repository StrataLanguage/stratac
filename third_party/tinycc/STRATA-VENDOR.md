...
changes set the version to `0.9.28-strata`, enable `CONFIG_TCC_PREDEFS` so the
CMake-generated `tccdefs_.h` is embedded, and enable `CONFIG_TCC_SEMLOCK` for
parallel compiler instances. They should remain limited to that configuration
and clearly identified integration files so the tree can be refreshed by
replacing it from the Snow copy and reapplying these documented deltas.
