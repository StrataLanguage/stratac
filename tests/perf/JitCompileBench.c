...
static bool WriteText(FixtureWriter* writer, const char* text)
{
    ...
    if (fwrite(text, 1, length, writer->file) != length)
    {
        return false;
    }
    writer->bytes += length;
...
static bool GenerateFixture(const char* directory, size_t mib, FixtureShape shape, Fixture* fixture)
{
    ...
    else if (shape == ShapeControlFlow)
    {
        ...
        for (size_t i = 0; ok && i < operations; ++i)
        {
            ok = WriteText(&writer, "  if ((total & 1) == 0) { total = total + 3; } "
                                    "else { total = total + 1; }\n");
        }
        ok = ok && WriteText(&writer, "  return total;\n}\n");
    ...
    {
        ok = WriteText(&writer, "struct Pair { int a; int b; };\n"
                                "int entry() {\n  Pair p = {.a = 0, .b = 0};\n");
        for (size_t i = 0; ok && i < operations; ++i)
        {
    ...
    {
        ok = WriteText(&writer, "int mix(int a, int b) { return a + b; }\n"
    ...
    {
        ok = WriteText(&writer, "uint hot(uint state, int rounds) {\n"
    ...
    {
        ok = WriteText(&writer, "struct HotPair { uint a; uint b; };\n"
    ...
    {
        ok = WriteText(&writer, "uint mix(uint a, uint b) { return a * 1664525u + b; }\n"
                                "uint hot(uint state, int rounds) {\n"
                                "  for (int i = 0; i < rounds; i++) {\n"
...
static Module* ParseSource(const char* source, size_t length, const char* name, Arena* arena, DiagnosticEngine* diag,
...
static Timing CompileSource(const char* source, size_t length, const char* name, Backend backend, int expected)
{
    ...
    if (!DiagHasErrors(&diag))
    {
        backendTiming = CompileAst(module, backend, expected);
    }
...
static Summary Summarize(const double* input, size_t count)
{
    ...
    result.median = count & 1 ? values[count / 2] : (values[count / 2 - 1] + values[count / 2]) * 0.5;
...
static bool LoadScript(Module* module, Backend backend, LoadedScript* script)
{
    ...
    if (backend == BackendLlvm)
    {
        ...
        if (loaded)
        {
            script->hot = (uint32_t (*)(uint32_t, int))(uintptr_t)LLVMJitGetAddress(&script->llvm, "hot");
        }
        BuiltModuleDispose(&built);
    }
    else
...
static uint32_t ReferenceHot(FixtureShape shape, uint32_t state, int rounds)
{
    ...
    for (int i = 0; i < rounds; ++i)
    {
        ...
        case ShapeArithmetic:
            ...
            break;
        case ShapeControlFlow:
            state = (state & 1u) == 0u ? (state >> 1) ^ UINT32_C(277803737) : state * 3u + 1u;
            state ^= state << 7;
            break;
        case ShapeStructs:
...
static double RunHotBatch(uint32_t (*hot)(uint32_t, int), size_t calls, int rounds, uint32_t seed, uint32_t* resultOut)
{
    uint32_t result = seed;
    double start = NowSeconds();
    for (size_t i = 0; i < calls; ++i)
    {
        result = hot(result, rounds);
    }
...
static size_t CalibrateRuntime(uint32_t (*hot)(uint32_t, int), int rounds)
{
    ...
    for (int attempt = 0; attempt < 12; ++attempt)
    {
        ...
        if (elapsed >= targetSeconds || calls == maximumCalls)
        {
            return calls;
        }
...
static bool MeasureRuntime(FILE* csv, const Fixture* fixture, Module* ast, size_t samples, RuntimeReport* report)
{
    ...
    size_t commonCalls = calls[BackendLlvm] > calls[BackendTcc] ? calls[BackendLlvm] : calls[BackendTcc];
    ...
    bool ok = calls[BackendLlvm] > 0 && calls[BackendTcc] > 0 && elapsed[BackendLlvm] && elapsed[BackendTcc];
    ...
    for (size_t sample = 0; ok && sample < samples; ++sample)
    {
        ...
        for (int order = 0; order < 2; ++order)
        {
            ...
            double batch = RunHotBatch(scripts[backend].hot, calls[backend], rounds, seed, &results[backend]);
    ...
    if (ok)
    {
        ...
        double ratio = report->llvm.median > 0.0 ? report->tcc.median / report->llvm.median : 0.0;
        if (csv)
        {
            fprintf(csv, "%s,%zu,%zu,%zu,%zu,hot-runtime,LLVM,%.9f,%.9f,%.9f,,%zu,%.6f,%.3f,%zu\n", fixture->name,
                    fixture->bytes, fixture->lines, fixture->declarations, fixture->functions,
                    report->llvm.minimum * 1000.0, report->llvm.median * 1000.0, report->llvm.p95 * 1000.0, samples,
                    ratio, 1.0 / report->llvm.median, report->llvmCalls);
            fprintf(csv, "%s,%zu,%zu,%zu,%zu,hot-runtime,TinyCC,%.9f,%.9f,%.9f,,%zu,%.6f,%.3f,%zu\n", fixture->name,
                    fixture->bytes, fixture->lines, fixture->declarations, fixture->functions,
                    report->tcc.minimum * 1000.0, report->tcc.median * 1000.0, report->tcc.p95 * 1000.0, samples, ratio,
                    1.0 / report->tcc.median, report->tccCalls);
        }
    }
...
static void PrintRuntimeReports(const RuntimeReport* reports, size_t count)
{
    ...
    printf("%-22s %12s %12s %12s %12s %9s\n", "fixture", "LLVM ns/call", "TCC ns/call", "LLVM calls/s", "TCC calls/s",
           "ratio");
    for (size_t i = 0; i < count; ++i)
    {
        ...
        printf("%-22s %12.1f %12.1f %12.0f %12.0f %8.2fx\n", report->fixtureName, report->llvm.median * 1000000000.0,
               report->tcc.median * 1000000000.0, 1.0 / report->llvm.median, 1.0 / report->tcc.median, ratio);
    }
}

static void PrintPair(FILE* csv, const Fixture* fixture, const char* view, const double* llvm, const double* tcc,
                      size_t iterations)
{
    ...
    printf("%-22s %-18s %9.3f %9.3f %9.3f %9.3f %8.2fx\n", fixture->name, view, llvmSummary.median * 1000.0,
           tccSummary.median * 1000.0, mib / llvmSummary.median, mib / tccSummary.median, ratio);

    if (csv)
    {
        fprintf(csv, "%s,%zu,%zu,%zu,%zu,%s,LLVM,%.6f,%.6f,%.6f,%.3f,%zu,%.6f,,\n", fixture->name, fixture->bytes,
                fixture->lines, fixture->declarations, fixture->functions, view, llvmSummary.minimum * 1000.0,
                llvmSummary.median * 1000.0, llvmSummary.p95 * 1000.0, mib / llvmSummary.median, iterations, ratio);
        fprintf(csv, "%s,%zu,%zu,%zu,%zu,%s,TinyCC,%.6f,%.6f,%.6f,%.3f,%zu,%.6f,,\n", fixture->name, fixture->bytes,
                fixture->lines, fixture->declarations, fixture->functions, view, tccSummary.minimum * 1000.0,
                tccSummary.median * 1000.0, tccSummary.p95 * 1000.0, mib / tccSummary.median, iterations, ratio);
    }
}

static bool MeasureView(FILE* csv, const Fixture* fixture, const char* source, size_t length, Module* ast,
                        const char* view, size_t iterations)
{
...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            fprintf(stderr, "usage: strata_jit_bench [--quick] [--runtime-only] [--sizes 1,5,20] "
    ...
    if (csv)
    {
        fprintf(csv, "fixture,bytes,lines,declarations,functions,view,backend,min_ms,median_ms,"
    ...
    printf("Strata JIT benchmark: build=%s system=%s arch=%s compiler=%s LLVM=%s TinyCC=0.9.28-strata seed=20260731\n",
           STRATA_BUILD_TYPE, STRATA_SYSTEM_NAME, STRATA_SYSTEM_PROCESSOR, STRATA_C_COMPILER, strataLLVMVersion());
    if (!runtimeOnly)
    {
        printf("%-22s %-18s %9s %9s %9s %9s %9s\n", "fixture", "view", "LLVM ms", "TCC ms", "LLVM MiB/s", "TCC MiB/s",
               "ratio");
    }
    ...
    for (size_t s = 0; ok && s < sizeCount; ++s)
    {
        for (FixtureShape shape = ShapeArithmetic; ok && shape <= ShapeOverloads; ++shape)
        {
            ...
            Module* ast = ParseSource(source, length, fixture.name, &astArena, &astDiag, &astManager);
            ok = !DiagHasErrors(&astDiag);
            if (ok && !runtimeOnly)
            {
                ok = MeasureView(csv, &fixture, source, length, ast, "file-to-callable", iterations);
            }
            if (ok && !runtimeOnly)
            {
                ok = MeasureView(csv, &fixture, source, length, ast, "source-to-callable", iterations);
            }
            if (ok && !runtimeOnly)
            {
                ok = MeasureView(csv, &fixture, source, length, ast, "ast-to-callable", iterations);
            }
            if (ok)
            {
                ok = MeasureRuntime(csv, &fixture, ast, iterations, &runtimeReports[runtimeReportCount]);
            }
            if (ok)
            {
                runtimeReportCount++;
