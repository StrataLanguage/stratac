...
STRATA_TEST(jit_runs_resolved_overloads)
{
    ...
    int (*ei)(void) = (int (*)(void))strataJitGetFunction(jit, "entry_i");
    float (*ef)(void) = (float (*)(void))strataJitGetFunction(jit, "entry_f");
    STRATA_CHECK(ei != NULL);
    STRATA_CHECK(ef != NULL);
    if (ei)
    {
