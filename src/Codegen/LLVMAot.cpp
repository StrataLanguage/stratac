...
bool emitNativeFile(const BuiltModule& bm, const std::string& path,
    ...
    if (!bm.mod) { errorMessage = "no module to emit"; return false; }
    ...
    if (LLVMGetTargetFromTriple(triple, &target, &err)) {
        errorMessage = std::string("unknown target triple '") + triple + "': " +
                       (err ? err : "(no message)");
    ...
    LLVMTargetMachineRef tm = LLVMCreateTargetMachine(
        target, triple, "" /* host CPU */, "" /* features */,
        LLVMCodeGenLevelDefault, LLVMRelocDefault, LLVMCodeModelDefault);
    LLVMDisposeMessage(triple);
    if (!tm) { errorMessage = "could not create target machine"; return false; }
