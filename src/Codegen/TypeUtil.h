...
struct MappedType {
    ...
    bool isVector() const noexcept { return vec > 1; }
};
...
inline MappedType mapType(const TypeName& t) {
    ...
    if (split < base.size() && split > 0) {
        ...
        if ((maybe == "float" || maybe == "int" || maybe == "uint" ||
             maybe == "half" || maybe == "double" || maybe == "bool") &&
            v >= 1 && v <= 4) {
            base = maybe;
            vsize = v;
        }
    ...
    auto set = [&](bool flt, bool uns, int b, const char* ir) {
        ...
        m.vec = vsize;
        if (vsize == 1) m.ir = ir;
        else m.ir = "<" + std::to_string(vsize) + " x " + ir + ">";
    };

    if (base == "void") {
        m.valid = true; m.isVoid = true; m.ir = "void"; m.elemIr = "void";
    } else if (base == "bool")   set(false, false, 1, "i1");
    else if (base == "int")      set(false, false, 32, "i32");
    else if (base == "uint")     set(false, true,  32, "i32");
    else if (base == "half")     set(true,  false, 16, "half");
    else if (base == "float")    set(true,  false, 32, "float");
    else if (base == "double")   set(true,  false, 64, "double");
    else m.valid = false;
