    ...
    if (strcmp(n->callee, "array_push") == 0)
    {
        ...
        if (elemTd.isBox && !elemTd.boxInner && valNode->kind == NodeStrLiteral)
        {
            ...
            for (size_t ci = 0; ci < copyLen; ci++)
            {
                ...
                LLVMValueRef ba[1] = {ciVal};
                LLVMValueRef byte
                    = LLVMBuildLoad2(b->m_builder, i8Ty, LLVMBuildGEP2(b->m_builder, i8Ty, v.value, ba, 1, "s"), "b");
                LLVMBuildStore(b->m_builder, byte, LLVMBuildGEP2(b->m_builder, i8Ty, heap, ba, 1, "d"));
            }
            ...
            else if (elemTd.isBox)
            {
                ...
                if (eNode->kind == NodeStrLiteral)
                {
                    ...
                    for (size_t ci = 0; ci < copyLen; ci++)
                    {
                        LLVMValueRef ciVal = LLVMConstInt(I64Ty(b), (unsigned long long)ci, 0);
                        LLVMValueRef ba[1] = {ciVal};
                        LLVMValueRef byte = LLVMBuildLoad2(b->m_builder, i8Ty,
                                                           LLVMBuildGEP2(b->m_builder, i8Ty, v.value, ba, 1, "s"), "b");
                        LLVMBuildStore(b->m_builder, byte, LLVMBuildGEP2(b->m_builder, i8Ty, heap, ba, 1, "d"));
                    }
                    LLVMBuildStore(b->m_builder, heap, elemAddr);
                }
        ...
        if (r->value)
        {
            ...
            else
            {
                ...
                if (v.typeDesc.isBox && !v.typeDesc.boxInner && r->value->kind == NodeStrLiteral)
                {
                    ...
                    StrataAllocFn(b);
                    LLVMValueRef heap
                        = LLVMBuildCall2(b->m_builder, b->m_allocFnType, b->m_allocFn, args2, 1, "strret");
                    LLVMTypeRef i8Ty = LLVMInt8TypeInContext(b->m_ctx);
                    ...
                    for (size_t ci = 0; ci < copyLen; ci++)
                    {
                        ...
                        LLVMValueRef dstIdx[1] = {ciVal};
                        LLVMValueRef byteVal = LLVMBuildLoad2(
                            b->m_builder, i8Ty, LLVMBuildGEP2(b->m_builder, i8Ty, srcGep, srcIdx, 1, "srcr"), "br");
                        LLVMBuildStore(b->m_builder, byteVal,
                                       LLVMBuildGEP2(b->m_builder, i8Ty, dstGep, dstIdx, 1, "dstr"));
                    }
                    v = ValueMake(heap, b->m_curRet);
                }
            ...
            if (varDecl->init)
            {
                ...
                else if (isStrLiteral)
                {
                    ...
                    for (size_t ci = 0; ci < copyLen; ci++)
                    {
                        ...
                        LLVMValueRef dstIdx[1] = {ciVal};
                        LLVMValueRef byteVal = LLVMBuildLoad2(
                            b->m_builder, i8Ty, LLVMBuildGEP2(b->m_builder, i8Ty, srcGep, srcIdx, 1, "src"), "b");
                        LLVMBuildStore(b->m_builder, byteVal,
                                       LLVMBuildGEP2(b->m_builder, i8Ty, dstGep, dstIdx, 1, "dst"));
                    }
                }
